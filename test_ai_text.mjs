...
async function run() {
  ...
  snapshot.forEach(doc => {
    const text = doc.data().transcript || doc.data().content || "";
    if (text.includes("AI:") || text.includes("System:") || text.includes("Assistant:") || text.includes("בוט")) {
      console.log("FOUND AI in ID:", doc.id);
