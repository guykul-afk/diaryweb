...
export default function PersonalityAnalysisView({ uid }) {
  ...
  const agentMetaData = {
    ...
    fareast: { title: 'פילוסופיית המזרח הרחוק', icon: Sparkles, desc: 'ניתוח המבוסס על זן-בודהיזם, דאואיזם ואיורוודה - שחרור מהיאחזויות, קבלת המציאות, איזון וזרימה (Wu Wei)' }
  };
  ...
  const renderBoardroomDialogue = (reportsData) => {
    ...
    const fareastReport = reportsData.fareast || '';
    ...
    const dialogue = [
      ...
      {
        agent: 'fareast',
        name: 'מאסטר זן (המזרח הרחוק)',
        avatarColor: '#10b981',
        message: `בואו ניקח נשימה עמוקה ונראה את התמונה הרחבה באמת. מהפרספקטיבה של חוכמת המזרח, הקונפליקטים הללו נובעים מניסיון נוקשה לשלוט במציאות (אגו) במקום לזרום איתה (Wu Wei). כפי שעולה מהניתוח שלי:
        
        "${fareastReport.split('.')[0] || 'קיימת היאחזות חזקה בתוצאות המייצרת סבל מיותר.'}."
        
        התרגול כאן צריך להיות התמקדות בכאן ועכשיו, שחרור הצורך לאישור חיצוני, והבנת הארעיות (Impermanence) של רגשות אלו.`
      }
    ];
...
חיזוק משמר: הסרת החרדה בטווח הקצר (חיזוק שלילי) וקבלת שבחים מהסביבה בטווח הארוך (חיזוק חיובי).`,
      ...
      fareast: `ניכר מאבק מתמיד במציאות וחוסר קבלה של כאב כאופציה טבעית (דוקהה). קיימת היאחזות חזקה (Upadana) בהגדרת העצמי כ"עובד מצטיין", מה שמייצר סבל מיותר כאשר דימוי זה מאוים. ישנה תנודה בין איכויות ראג'סיות (פעילות יתר, תחרותיות) לתאמסיות (עייפות והימנעות), תוך חוסר במצב מנטלי סאטווי (שלווה ובהירות). הפתרון טמון בפיתוח של תרגול נוכחות (Mindfulness), עשייה ללא היקשרות לתוצאה (Karma Yoga), והליכה בדרך האמצע.`
    }
  };
}
